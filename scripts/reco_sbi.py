...
timeframes = [400, 401, 402]  # which images to reconstruct
...
for i in range(sino.shape[1]):  # go through timeframes one by one
    sino_t = np.transpose(sino[:, i], [0, 2, 1])
    ...
    vol_id, vol_geom = recon.backward(
        ...
        proj_geom,
        algo='sirt',
        voxels=(100, 100, 500),  # (300, 300, 1500) for better resolution
        voxel_size=5.5 / 100,  # 5.5 cm / 300 voxels
        iters=200,
        min_constraint=0.0,
        max_constraint=1.0,
        col_mask=True)
