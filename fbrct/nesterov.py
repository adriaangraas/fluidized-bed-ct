...
class AcceleratedGradientPlugin(astra.plugin.base):
    ...
    def initialize(self, cfg, liptschitz=1, MinConstraint=None,
                   MaxConstraint=None):
    ...
    def run(self, its):
        ...
        print('running', str(its),
              'iterations of Accelerated Gradient plugin.', flush=True)
        for i in range(its):
            ...
            descent_direction = gradient - tau / nu * (
                x_apgd - x_old) + tau * (NRMx - NRMx_old);

            # update x
            x_old[:] = x_apgd[:]
            x_apgd -= nu * descent_direction
            if self.min_constraint is not None or self.max_constraint is not None:
                x_apgd.clip(min=self.min_constraint, max=self.max_constraint,
                            out=v)
